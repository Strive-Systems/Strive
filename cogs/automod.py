...
class AutoModCommandCog(commands.Cog):
            ...
            if word in content:
                ...
                try:
                    await message.author.send(
                        f"Your message in **{message.guild.name}** contained inappropriate content and was removed."
                    )
                except discord.Forbidden:
                    await message.channel.send(
                        f"{message.author.mention}, your message contained inappropriate content and was removed."
                    )

    @commands.hybrid_command(
        name="addword", description="Adds a word to the banned words list (Admin only)"
    )
    ...
    async def addword(self, ctx: StriveContext, word: str):
        ...
        await ctx.send_success(
            f"The word `{word}` has been added to the banned words list."
        )

    @commands.hybrid_command(
        name="removeword",
        description="Removes a word from the banned words list (Admin only)",
    )
    @commands.has_permissions(administrator=True)
    async def removeword(self, ctx: StriveContext, word: str):
        ...
        await self.fetch_banned_words()
        await ctx.send_success(
            f"The word `{word}` has been removed from the banned words list."
        )

    @commands.hybrid_command(
        name="listwords", description="Lists the banned words for this guild"
    )
    async def listwords(self, ctx: StriveContext):
        ...
        if not banned_words_list:
            await ctx.send_error(
                f"No banned words found for this server.", ephemeral=True
            )
            return

        banned_words_str = ", ".join(banned_words_list)
        embed_color = constants.strive_embed_color_setup()
        embed = AutoModListWordsEmbed(
            guild_name=ctx.guild.name, banned_words=banned_words_str, color=embed_color
        )
