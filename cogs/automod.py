...
class AutoModCommandCog(commands.Cog):
    ...
    async def addword(self, ctx: commands.Context, word: str):
        ...
        await self.fetch_banned_words()
        await ctx.send(f"<:success:1326752811219947571> The word `{word}` has been added to the banned words list.")
    ...
    async def removeword(self, ctx: commands.Context, word: str):
        ...
        await self.fetch_banned_words()
        await ctx.send(f"<:success:1326752811219947571> The word `{word}` has been removed from the banned words list.")
    ...
    async def listwords(self, ctx: commands.Context):
        ...
        if not banned_words_list:
            await ctx.send("<:error:1326752911870660704> No banned words found for this server.", ephemeral=True)
            return
