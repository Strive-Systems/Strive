...
class OnGuildJoin(commands.Cog):
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        embed.add_field(name='Owner Info', value=f"<@{str(guild.owner_id)}>(``{str(guild.owner_id)}``)", inline=False)
        embed.add_field(name='Current Server Count', value=str(len(self.strive.guilds)), inline=False)
