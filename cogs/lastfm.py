...
class LastFMCommandCog(commands.Cog):
    ...
    @commands.hybrid_group(
        name="lastfm", aliases=["lf"], description="Last.fm commands", fallback="help"
    )
    ...
    async def lastfm_set(self, ctx: StriveContext, username: str):
        ...
        try:
            ...
            embed.add_field(
                name="Total Scrobbles", value=str(total_scrobbles), inline=True
            )
            embed.add_field(
                name="Account Created",
                value=f"<t:{registered_timestamp}:R>",
                inline=True,
            )
    ...
    async def lastfm_topartists(self, ctx: StriveContext):
        ...
        try:
            top_artists = await self.lastfmhandler.get_top_artists(
                check["username"], 10
            )
            description = []
            for i, artist in enumerate(top_artists["topartists"]["artist"]):
                description.append(
    ...
    async def lastfm_topsongs(self, ctx: StriveContext):
        ...
        try:
            ...
            description = []
            for i, track in enumerate(top_tracks["toptracks"]["track"]):
                description.append(
                    f"`{i+1}` **{track['name']}** by {track['artist']['name']} - {track['playcount']} plays"
                )
    ...
    async def lastfm_topalbums(self, ctx: StriveContext):
        ...
        try:
            ...
            description = []
            for i, album in enumerate(top_albums["topalbums"]["album"]):
                description.append(
                    f"`{i+1}` **{album['name']}** by {album['artist']['name']} - {album['playcount']} plays"
                )
    ...
    @lastfm.command(
        name="whoknows", aliases=["wk"], description="View who knows an artist"
    )
    async def lastfm_whoknows(self, ctx: StriveContext, *, artist: str = None):
        ...
        listeners = []
        async for user in lastfm.find(
            {"discord_id": {"$in": [m.id for m in ctx.guild.members]}}
        ):
            member = ctx.guild.get_member(user["discord_id"])
            if member:
                plays = await self.lastfmhandler.get_artist_playcount(
        ...
        for i, (member, plays) in enumerate(listeners[:10]):
            description.append(
                f"{'🎵' if i == 0 else f'`{i+1}`'} {member.mention} - **{plays}** plays"
            )
    ...
    @commands.hybrid_command(
        aliases=["np", "fm"],
        description="View your currently playing track",
        with_app_command=True,
        extras={"category": "LastFM"},
    )
    async def nowplaying(self, ctx: StriveContext, member: discord.Member = None):
        ...
        if not check:
            return await ctx.send_error(
                f"{'You don' if member == ctx.author else f'**{member}** doesn'}'t have a Last.fm account connected."
            )
