...
class ModerationCommandCog(commands.Cog):
    ...
    @commands.hybrid_command(
        description="You can run this command to warn a user in your server.",
        ...
        *,
        ...
        if await ModerationCommandCog.is_blacklisted_or_admin(ctx, member):
            ...
            await ctx.send_error(
                f"You cannot warn {member.mention} because they are an admin or bypassed from moderation."
            )
            ...
            try:
                ...
                await ctx.send_error(
                    f"Unable to send a DM to {member.mention}; warning the user in the server."
                )
            ...
            await ctx.send_success(
                f"**Case #{case_id} - {member}** has been warned for {reason}."
            )

    @commands.hybrid_command(
        name="ban",
        description="Ban command to ban members from your server.",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    ...
    async def ban(
        ...
        member: discord.User,
        *,
        reason: str = "Nothing was provided",
    ):

        if await ModerationCommandCog.is_blacklisted_or_admin(ctx, member):

            await ctx.send_error(
                f"You cannot ban {member.mention} because they are an admin or bypassed from moderation."
            )
            ...
            try:
                if fetched_member.top_role >= ctx.author.top_role:
                    return await ctx.send_error(
                        f"You cannot ban a member with an equal or higher role!"
                    )
            ...
            await ctx.send_success(
                f"**Case #{case_id} - {fetched_member}** has been banned for {reason}."
            )

    @commands.hybrid_command(
        name="unban",
        description="Unban command to unban members from your server.",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    @commands.has_guild_permissions(ban_members=True)
    async def unban(
        ...
        *,
        ...
        try:
            ...
            case_id = await get_next_case_id(ctx.guild.id)
            await ctx.send_success(
                f"**Case #{case_id} - {user_to_unban}** has been unbanned for {reason}."
            )
    ...
    @commands.hybrid_command(
        description="Softban a user, deleting their messages from the server.",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    ...
    async def softban(
        ...
        if await ModerationCommandCog.is_blacklisted_or_admin(ctx, member):
            await ctx.send_error(
                f"You cannot softban {member.mention} because they are an admin or bypassed from moderation."
            )
            ...
            await ctx.send_success(
                f"**Case #{case_number} - Successfully softbanned {member.mention}** for: {reason}"
            )
            await asyncio.sleep(2)
            await ctx.guild.unban(member)

    @commands.hybrid_command(
        description="Mute/Timeout a certain user",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    ...
    async def mute(
        ...
        *,
        ...
        if await ModerationCommandCog.is_blacklisted_or_admin(ctx, member):
            await ctx.send_error(
                f"You cannot mute {member.mention} because they are an admin or bypassed from moderation."
            )
            ...
            if unit == "m":
                ...
                return await ctx.send_error(
                    "Invalid time unit. Use `m`, `h`, `s`, or `d`."
                )
            ...
            await ctx.send_success(
                f"**{member.name}** has been muted until {formatted_time}!"
            )

    @commands.hybrid_command(
        description="Remove timeout from a certain user",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    @commands.has_permissions(moderate_members=True)
    async def unmute(
        ...
        *,
        ...
        try:
            if member.top_role >= ctx.author.top_role:
                return await ctx.send_error(
                    "You cannot unmute a member with an equal or higher role!"
                )
        except AttributeError:
            pass
    ...
    @commands.hybrid_command(
        description="You can run this command to kick a user in your server.",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    ...
    async def kick(
        ...
        *,
        ...
        if await ModerationCommandCog.is_blacklisted_or_admin(ctx, member):
            await ctx.send_error(
                f"You cannot kick {member.mention} because they are an admin or bypassed from moderation."
            )
            ...
            try:
                ...
                await ctx.send_error(
                    f"Unable to send a DM to {member.mention}; kicking the user in the server."
                )
            ...
            await ctx.send_success(
                f"**Case #{case_id} - {member}** has been kicked for {reason}."
            )
    ...
    async def view(self, ctx: StriveContext, caseid: int):
        ...
        if case_info:
            ...
            member_value = (
                f"<@{case_info.get('user_id')}> (`{case_info.get('user_id')}`)"
            )
            ...
            try:
                member: discord.Member = await self.strive.fetch_user(
                    case_info.get("user_id")
                )
    ...
    async def void(self, ctx: StriveContext, *, caseid: int):
        case_info = await cases.find_one_and_update(
            {"case_id": caseid, "guild_id": ctx.guild.id},
            {"$set": {"status": "cleared"}},
        )
    ...
    @commands.hybrid_group(
        description="Allows the lookup, transferring and modification of modlogs."
    )
    ...
    async def view(self, ctx: StriveContext, user: discord.User):
        ...
        async for result in results:
            if result.get("status") == "active":
                ...
                embed.add_field(
                    name=f"Case #{result.get('case_id')} | {result.get('type').title()}",
                    value=(
        ...
        embed.set_author(
            ...
            icon_url=(
                user_obj.avatar.url if user_obj.avatar else user_obj.default_avatar.url
            ),
        )
    ...
    async def transfer(self, ctx, olduser: discord.Member, newuser: discord.Member):
        ...
        if failed_cases:
            await ctx.send_error(
                f"The following cases could not be updated: {', '.join(map(str, failed_cases))}"
            )

        await ctx.send_success(
            f"All moderation logs for **{olduser.name}** have been transferred to **{newuser.name}**."
        )
    ...
    async def clear(self, ctx, member: discord.Member):
        ...
        if failed_cases:
            await ctx.send_error(
                f"The following cases could not be cleared: {', '.join(map(str, failed_cases))}"
            )

        await ctx.send_success(
            f"All moderation logs have been cleared for **{member.name}**."
        )
    ...
    async def banlist(self, ctx):
        ...
        class BanDropdown(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                ban = next(
                    (ban for ban in ban_list if str(ban.user.id) == self.values[0]),
                    None,
                )
                ...
                if ban:
                    ...
                    embed.add_field(
                        name="User", value=f"{ban.user} (`{ban.user.id}`)", inline=False
                    )
                    embed.add_field(
                        name="Reason",
                        value=ban.reason or "No reason provided",
                        inline=False,
                    )
    ...
    async def slowmode(
        ...
        if duration == "0s":
            try:
                ...
                await ctx.send_error(
                    f"I don't have permission to modify slowmode in {channel.mention}."
                )
                return
        ...
        try:
            ...
            await ctx.send_error(
                "Invalid duration format; use format like: 10s, 5m, 1h"
            )
            return

        if seconds > 21600:  # discord's max slowmode is 6 hours
            await ctx.send_error(
                "Slowmode cannot be longer than 6 hours (21600 seconds)."
            )
            return
        ...
        try:
            ...
            await ctx.send_success(
                f"Slowmode set to **{duration.lower()}** in {channel.mention}."
            )
    ...
    @commands.hybrid_command(
        description="Remove member's attach files & embed links permission",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    @commands.has_permissions(moderate_members=True)
    ...
    @commands.hybrid_command(
        description="Restore member's attach files & embed links permission",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    @commands.has_permissions(moderate_members=True)
    ...
    @commands.hybrid_command(
        description="Remove member's reaction & external emotes permission",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    @commands.has_permissions(moderate_members=True)
    async def rmute(
        ...
        await ctx.channel.set_permissions(member, overwrite=overwrite)
        await ctx.send_success(
            f"Removed reaction/emoji permissions from {member.mention}"
        )

    @commands.hybrid_command(
        description="Restore member's reaction & external emotes permission",
        with_app_command=True,
        extras={"category": "Moderation"},
    )
    @commands.has_permissions(moderate_members=True)
    async def runmute(
        ...
        overwrite = discord.PermissionOverwrite(
            add_reactions=None, use_external_emojis=None
        )
        await ctx.channel.set_permissions(member, overwrite=overwrite)
        await ctx.send_success(
            f"Restored reaction/emoji permissions for {member.mention}"
        )
    ...
    async def strip(self, ctx, member: discord.Member):
        ...
        if member.top_role >= ctx.author.top_role:
            await ctx.send_error(
                "You cannot strip roles from someone with an equal or higher role than you."
            )
            return
        ...
        for role in dangerous_roles:
            ...
            try:
                await member.remove_roles(
                    role, reason=f"Role strip command used by {ctx.author}"
                )
        ...
        if removed_roles:
            await ctx.send_success(
                f"Removed **{len(removed_roles)}** dangerous roles from {member.mention}\n-# <:right:1338812220825665650>  {', '.join(removed_roles)}"
            )
        else:
